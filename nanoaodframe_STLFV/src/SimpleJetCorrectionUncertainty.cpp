...
//      sserr<<"SimpleJetCorrectionUncertainty"<<" ERROR: "<<" bin variables out of range " << std::endl;
...
float SimpleJetCorrectionUncertainty::uncertaintyBin(unsigned fBin, float fY, bool fDirection) const
{
  ...
  {
      std::stringstream sserr;
      sserr<<"SimpleJetCorrectionUncertainty"<<" ERROR: "<< "wrong # of parameters: multiple of 3 expected, "<<p.size()<< " got" << std::endl;
      throw std::runtime_error(sserr.str());
  }
